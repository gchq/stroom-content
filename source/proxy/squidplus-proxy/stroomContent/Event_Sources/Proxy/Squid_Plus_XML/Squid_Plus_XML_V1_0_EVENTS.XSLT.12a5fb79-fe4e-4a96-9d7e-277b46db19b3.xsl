...
<xsl:stylesheet xmlns="event-logging:3" xmlns:stroom="stroom" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="3.0">
  ...
  <!--
  Change Log:
  20211014 - Revert to v3.5.2 prior to release on github.com. Conversion still valid.
  20210918 - Conversion for V4 of schema where Location/TimeZone -> ../Data[@Name='TimeZone']
  20200109 - Cater for ports of the form <host> '/' <portnumber>
  -->
  ...
  <xsl:template name="event_source">
    ...
    <EventSource>
      ...
      <Generator>
        <xsl:choose>
          <xsl:when test="stroom:meta('Version')">
      ...
      <Client>
        ...
        <xsl:call-template name="emitPort">
          <xsl:with-param name="port" select="cPort" />
        </xsl:call-template>
      </Client>
      ...
      <Server>
        ...
        <xsl:call-template name="emitPort">
          <xsl:with-param name="port" select="sPort" />
        </xsl:call-template>
      </Server>
  ...
  <xsl:template name="event_detail">
    <EventDetail>
      ...
      -->
      <TypeId>
        <xsl:value-of select="concat('Squid-', rMethod, '-', substring-before(rURL, ':'))" />
        <xsl:if test="matches(rURL, '.+\?.+')">-Query</xsl:if>
      </TypeId>
      <xsl:choose>
  ...
  <xsl:template name="setPayload">
    <xsl:variable name="v" select="stroom:parse-uri(rURL)" />
    ...
    <xsl:if test="$v/*:query != ''">
      <Criteria>
        <DataSources>
          <DataSource>
            <xsl:value-of select="concat($v/*:scheme, '://', $v/*:host, $v/*:path, ':', $v/*:port)" />
          </DataSource>
        </DataSources>
        <Query>
          <Raw>
            <xsl:value-of select="$v/*:query" />
          </Raw>
        </Query>
      </Criteria>
    </xsl:if>
  ...
  <xsl:template name="emitPort">
    ...
    <xsl:choose>
      ...
      <!-- If contains a '/' then probably of the form host/port -->
      <xsl:when test="contains($port, '/')">
        <xsl:if test="substring-after($port, '/') != '-'">
          <Port>
